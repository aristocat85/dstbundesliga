...
class AwardService:
    ...
    def get_all(self):
        ...
        if self.waivers.count() > 0:
            awards.extend(
                [self.get_most_wanted_waiver(), self.get_highest_bid_waiver()]
            )
    ...
    def get_highscorer(self):
        ...
        if mpo.points_one >= mpt.points_two:
            ...
            most_points_roster = self.rosters.get(
                league__sleeper_id=mpo.league_id, roster_id=mpo.roster_id_one
            )
        else:
            most_points_score = mpt.points_two
            most_points_roster = self.rosters.get(
                league__sleeper_id=mpt.league_id, roster_id=mpt.roster_id_two
            )
    ...
    def get_lowscorer(self):
        ...
        if mpo.points_one <= mpt.points_two:
            ...
            least_points_roster = self.rosters.get(
                league__sleeper_id=mpo.league_id, roster_id=mpo.roster_id_one
            )
        else:
    ...
    def get_narrow_victory(self):
        ...
        roster_one = self.rosters.get(
            league__sleeper_id=head_to_head.league_id,
            roster_id=head_to_head.roster_id_one,
        )
        roster_two = self.rosters.get(
            league__sleeper_id=head_to_head.league_id,
            roster_id=head_to_head.roster_id_two,
        )
    ...
    def get_blowout_victory(self):
        ...
        roster_one = self.rosters.get(
            league__sleeper_id=blowout.league_id, roster_id=blowout.roster_id_one
        )
        roster_two = self.rosters.get(
            league__sleeper_id=blowout.league_id, roster_id=blowout.roster_id_two
        )
    ...
    def get_shootout(self):
        shootout = (
            self.narrow_matchups.filter(point_difference__lte=10)
            .annotate(points_sum=F("points_one") + F("points_two"))
            .order_by("-points_sum")
            .first()
        )
        ...
        if not shootout:
            shootout = (
                self.narrow_matchups.filter(point_difference__lte=20)
                .annotate(points_sum=F("points_one") + F("points_two"))
                .order_by("-points_sum")
                .first()
            )
        ...
        if not shootout:
            shootout = (
                self.narrow_matchups.annotate(
                    points_sum=F("points_one") + F("points_two")
                )
                .order_by("-points_sum")
                .first()
            )
        ...
        roster_one = self.rosters.get(
            league__sleeper_id=shootout.league_id, roster_id=shootout.roster_id_one
        )
        roster_two = self.rosters.get(
            league__sleeper_id=shootout.league_id, roster_id=shootout.roster_id_two
        )
    ...
    def get_cffc_vs_affc(self):
        cffc_league_ids = (
            League.objects.get_active()
            .filter(conference="CFFC")
            .values_list("sleeper_id", flat=True)
        )
        affc_league_ids = (
            League.objects.get_active()
            .filter(conference="AFFC")
            .values_list("sleeper_id", flat=True)
        )

        points_cffc = (
            self.matchups.filter(league_id__in=cffc_league_ids)
            .aggregate(point_sum=Sum("points_one") + Sum("points_two"))
            .get("point_sum")
        )
        points_affc = (
            self.matchups.filter(league_id__in=affc_league_ids)
            .aggregate(point_sum=Sum("points_one") + Sum("points_two"))
            .get("point_sum")
        )
    ...
    def get_buli_leader(self):
        leader = Roster.objects.filter(
            league__season=Season.get_active(), league__level=1
        ).first()
    ...
    def get_cffc_leader(self):
        leader = Roster.objects.filter(
            league__season=Season.get_active(), league__conference="CFFC"
        ).first()
        if not leader:
            return None
    ...
    def get_affc_leader(self):
        leader = Roster.objects.filter(
            league__season=Season.get_active(), league__conference="AFFC"
        ).first()
        if not leader:
            return None
    ...
    def get_most_wanted_waiver(self):
        waiver_pickup = (
            self.waivers.values("player")
            .annotate(target_count=Count("player"))
...
class Award:
    ...
    def __init__(self, request, context, title=None, icon=None):
        ...
        context.update({"title": title, "icon": icon})
