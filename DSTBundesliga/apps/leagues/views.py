...
from DSTBundesliga.apps.leagues.tables import (
    LeagueTable,
    RosterTable,
    DraftsADPTable,
    NextDraftsTable,
...
def roster_list(request, league_id):
    ...
    return render(request, "leagues/roster_list.html", {"title": title, "table": table})
...
def level_detail(request, level=None, conference=None, region=None, season=None):
    ...
    leagues = [
        {
            ...
            "stats_link": reverse(
                "facts_and_figures_league", kwargs={"league_id": league.sleeper_id}
            ),
            "playoffs_link": reverse(
                "playoffs", kwargs={"league_id": league.sleeper_id}
            ),
            "draft_link": reverse(
                "draft-board", kwargs={"league_id": league.sleeper_id}
            )
            if league.draft.status != "pre_draft"
            else None,
        }
    ...
    return render(
        request,
        "leagues/level_detail.html",
        {"leagues": leagues, "header_logo": header_logo},
    )
...
def my_league(request):
    ...
    context = {
        "levels": [
            ...
            for level in all_leagues.order_by("level")
...
def draft_stats(request, position=None):
    drafts = Draft.objects.filter(league__season__active=True).filter(
        league__type=League.BUNDESLIGA
    )
    ...
    next_drafts_table = NextDraftsTable(
        drafts.exclude(start_time=None)
        .exclude(start_time__lte=datetime.utcnow().replace(tzinfo=pytz.utc))
        .exclude(status__in=["complete", "drafting", "paused"])
        .order_by("start_time", "league__level", "league__sleeper_name")[:10]
    )

    upset_and_value_picks = player_stats.filter(
        pick_count__gte=drafts_done * 0.8
    ).annotate(
        upset_value=ExpressionWrapper(
            F("adp") - F("highest_pick"), output_field=IntegerField()
        ),
        steal_value=ExpressionWrapper(
            F("lowest_pick") - F("adp"), output_field=IntegerField()
        ),
    )
    ...
    for upset_pick in upset_and_value_picks.order_by("-upset_value"):
        pick = (
            picks.filter(
                player__sleeper_id=upset_pick.player_id, pick_no=upset_pick.highest_pick
            )
            .annotate(
                adp=ExpressionWrapper(
                    Value(float(upset_pick.adp)), output_field=FloatField()
                )
            )
            .first()
        )
    ...
    for steal_pick in upset_and_value_picks.order_by("-steal_value"):
        pick = (
            picks.filter(
                player__sleeper_id=steal_pick.player_id, pick_no=steal_pick.lowest_pick
            )
            .annotate(
                adp=ExpressionWrapper(
                    Value(float(steal_pick.adp)), output_field=FloatField()
                )
            )
            .first()
        )
...
def player_stats(request, position=None):
    ...
    players = players.raw(
        "select * from (select id, first_name, last_name, position, points, games_played, points/games_played as avg_points from leagues_player as whatever left join (select player_id, sum(points) as points, Sum(Case when stats = '"
        "' then 0 when stats='{}' then 0 else 1 end) as games_played from leagues_statsweek group by player_id) as asdf on id=player_id %s) as player_data left join (select player_id, AVG(pick_no) as adp from leagues_pick group by player_id) as adp_picks on id=player_id order by  points desc, adp asc;"
        % pos_filter
    )
...
def draftboard(request, league_id):
    ...
    draft_order = sorted(
        [(draft.draft_order.get(owner.sleeper_id), owner) for owner in owners],
        key=lambda do: do[0],
    )

    return render(
        request,
        "stats/draftboard.html",
        {
...
def playoffs(request, league_id):
                    ...
                    "matchups": [
                        {
                            "roster_one": league.rosters.get(
                                roster_id=matchup.roster_id_one
                            )
                            if matchup.roster_id_one
                            else None,
                            "roster_two": league.rosters.get(
                                roster_id=matchup.roster_id_two
                            )
                            if matchup.roster_id_two
                            else None,
                        }
                        for matchup in playoff_matchups.filter(
                            round=round, bracket=bracket
                        )
                    ],
                }
                for round in sorted(
                    playoff_matchups.filter(bracket=bracket)
    ...
    context = {"league": league, "brackets": brackets}
...
def cl_quali(request):
    context = {}
    cl_quali_rosters = Roster.objects.filter(
        league__season=Season.get_active(), league__type=League.BUNDESLIGA
    ).order_by("-fpts", "-fpts_decimal")
...
def facts_and_figures(request):
    ...
    return render(
        request,
        "stats/facts_and_figures.html",
        {"current_week": week, "stats": stats, "awards": awards},
    )
...
def waiver_stats(request):
    waivers = WaiverPickup.objects.filter(
        season=Season.get_active(), changed_ts__gte=datetime.now() - timedelta(days=7)
    )

    waivers_for_sum = (
        waivers.values("player")
        .annotate(
            sum=Sum("bid"),
            count=Count("player"),
            avg=Avg("bid"),
            leagues=Count("roster__league", distinct=True),
        )
        .order_by("-sum")[:20]
    )
    ...
    waivers_for_sum_success = waivers.values("player").annotate(
        sum=Sum("bid"), count=Count("player"), avg=Avg("bid")
    )
...
class StatService:
    def __init__(self, week=None, league_id=None):
        ...
        self.rosters = Roster.objects.filter(
            league__season=Season.get_active(), league__type=League.BUNDESLIGA
        )
    ...
    def avg_faab(self):
        ...
        avg_faab_used = (
            rosters.aggregate(avg_faab_used=Avg("waiver_budget_used")).get(
                "avg_faab_used"
            )
            or 0
        )
        return "{faab}$".format(faab=int(100 - avg_faab_used))
    ...
    def avg_points_league(self):
        ...
        sum_points = matchups.aggregate(
            sum_points=Sum("points_one") + Sum("points_two")
        ).get("sum_points")
        return "{:.2f}".format(sum_points / rosters.count())

    def avg_points(self):
        sum_points = self.matchups.aggregate(
            sum_points=Sum("points_one") + Sum("points_two")
        ).get("sum_points")
    ...
    def league_ranking(self):
        leagues = (
            self.matchups.values("league_id")
            .annotate(sum_points=Sum("points_one") + Sum("points_two"))
            .order_by("-sum_points")
        )
        ranking = (
            list(leagues.values_list("league_id", flat=True)).index(self.league_id) + 1
        )
    ...
    def median_loosing_points(self):
        ...
        losing_values = [
            mu.points_one if mu.points_one < mu.points_two else mu.points_two
            for mu in matchups
        ]
